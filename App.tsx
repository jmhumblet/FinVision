...
import VariableIncomeSmoother from './components/VariableIncomeSmoother';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3 md:space-x-4">
            ...
            <div className="flex space-x-1 bg-slate-100 p-1 rounded-xl">
                ...
                <button
                  onClick={() => setCurrentView(AppView.VARIABLE_INCOME_SMOOTHER)}
                  className={`p-2 rounded-lg transition-all ${currentView === AppView.VARIABLE_INCOME_SMOOTHER ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
                  title="Variable Income Smoother"
                >
                  <Activity size={20} />
                </button>
            </div>
        ...
        {currentView === AppView.MONTHLY && monthlySummary ? (
          ...
          />
            ...
            <FinancialHealthDashboard
                ...
                timelineData={timelineData}
            />
        ) : currentView === AppView.VARIABLE_INCOME_SMOOTHER ? (
            <VariableIncomeSmoother
                transactions={transactions}
                projections={projections}
            />
        ) : (
