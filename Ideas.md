...
### User Story
As a user with multiple debts, I want to compare different payoff strategies (Snowball vs. Avalanche) so that I can choose the optimal path to become debt-free and visualize my payoff timeline.

### Acceptance Criteria
- System allows users to input multiple debts including balance, interest rate, and minimum payment.
- Users can toggle between Snowball (lowest balance first) and Avalanche (highest interest rate first) strategies.
- Visual timeline displays projected payoff dates for each debt and total debt-free date based on the chosen strategy.
- Users can simulate adding extra monthly payments to see how it accelerates the payoff timeline.
- Total interest paid is calculated and compared across both strategies.
